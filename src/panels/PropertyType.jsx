...
const TYPE_CHOICES = [
  ...
  { value: 'void', label: 'void', icon: <ElementSlotIcon size={14} /> },
  {
    value: 'any',
    label: 'any',
    hint: 'Unchecked value',
    icon: <SparkleIcon size={14} />,
  },
];
export function PropertyType({ type, onChange }) {
  const supported =
    TYPE_CHOICES.some((option) => option.value === type) ||
    literalOptions(type) !== undefined;
  const [mode, setMode] = useState(supported ? 'dropdown' : 'expression');
