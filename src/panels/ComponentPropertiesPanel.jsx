...
import { useComponentProperties } from './useComponentProperties.js';
...
import { PropertyGrip, movePropertyItem } from './PropertyReorder.jsx';
import { PropertiesIcon } from '../ui/PropertiesIcon.jsx';
...
export default function ComponentPropertiesPanel({
  ...
  const props = { projectPath, file, name, flushSave, onSavePhase, onSaved };
  const controller = useComponentProperties(props);
  const data =
    controller.state.kind === 'ready' ? controller.state.data : undefined;
  return (
    <section className="component-properties" aria-label="Component properties">
      ...
      {controller.state.kind === 'loading' && (
        <p className="property-help">Loading properties…</p>
      )}
      {controller.state.kind === 'error' && (
        <p role="alert">{controller.state.message}</p>
      )}
      {data && (
...
function PropertyPanelContent({ data, busy, save }) {
    ...
    <>
      {selection.kind === 'none' && (
        <button
          ...
          title={
            data.advanced
              ? 'Add declarations to this combined type in source'
              : 'Add property'
          }
...
function PropertySelectionEditor({ selection, data, busy, save, onClose }) {
  return (
    <>
      {selection.kind === 'property' && (
        <PropertyEditor
          ...
          access={
            busy
              ? 'saving'
              : propertyIsEditable(selection.value, data)
                ? 'editable'
                : 'readonly'
          }
          onClose={onClose}
    ...
    <div className="property-list list-field">
      {data.properties.length === 0 && (
        <p className="property-help">
          No properties yet. Add a property to define what each instance can
          customize.
        </p>
      )}
...
function propertyIsEditable(property, data) {
  return property.editing
    ? property.editing.kind === 'editable'
    : !data.advanced;
}
