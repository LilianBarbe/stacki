...
import { DragIcon } from '../ui/Icons.jsx';
// Drop targets are gaps before removal, matching the shared list gesture hook.
export function movePropertyItem(items, source, gap) {
  assert(
    items.length <= PROPERTY_LIMITS.fieldsMax,
    'Property list exceeds bounds',
  );
  assert(
    Number.isSafeInteger(source),
    'Property source index must be an integer',
  );
  ...
  assert(
    ...
    'Reordering must preserve the property count',
  );
  return next;
}
export function PropertyGrip({ label, index, count, disabled, onMove }) {
  return (
    <button
      type="button"
