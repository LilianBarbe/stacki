...
const onFilesChanged = (callback) => window.avb.onFsChanged(callback);
import {
  editComponentProperties,
  readComponentProperties,
} from '../componentPropertiesBridge.js';
export function useComponentProperties(props) {
...
function usePropertySnapshot(props, pending, setError) {
...
function usePropertyWatcher(props, { active, invalidateReads, reload }) {
  useEffect(() => {
    active.current = true;
...
function refreshSnapshot(previous, state) {
...
async function loadPropertiesPanel(props, reason) {
  try {
