...
- **Git workspaces** — click the project name in the title bar to search and open existing worktrees of the same repository, including those created in Conductor. The picker shows each folder and branch, marks the active workspace, and preserves the project subfolder in monorepos. Switching saves the current page, closes the current terminal and agent session, and reloads the editor in the selected workspace. Each opened workspace keeps its Astro server on its own local port; returning reconnects to the same server. The picker shows the running preview addresses. Closing the project or quitting Stacki stops all retained servers started by Stacki. Worktrees are also clickable in History.
- **Prepare workspaces** — choose **Prepare** beside an unprepared workspace to install its dependencies while continuing to edit the current project. Installations run one at a time at reduced process priority, reuse the npm cache when possible, and do not start additional previews. Opening a workspace during preparation waits for the same installation. First-time downloads and the preview's initial compilation still take time; reduced priority is not a CPU usage limit.
- **Code** — the rail's Code tab opens the file behind whatever the canvas has selected (the page, or the component being edited), scrolled to the selection with the rest of the file turned down until you click into it. It's a real editor: edits go straight to disk and the canvas, navigator and panels follow.
