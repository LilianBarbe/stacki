...
const { renderComponentPreviewPage } = require('./componentPreview.js');
...
// only our helpers — our own code, no load-time side effects — deliberately not
...
function writeMarkerConfig(projectPath, pushDevLog = () => {}) {
  try {
    ...
    const previewHelperPath = path
      ...
      .replace(`app.asar${path.sep}`, `app.asar.unpacked${path.sep}`);
    // Vite resolves symlinks before loading project files (e.g. /var on macOS).
    const previewRoots = [...new Set([projectPath, fs.realpathSync(projectPath)])];
...
const { componentPreviewPlugin } = require(${JSON.stringify(previewHelperPath)});
...
export default {
  ...
  vite: {
    ...(base.vite || {}),
    plugins: [componentPreviewPlugin(${JSON.stringify(previewRoots)}), avbMarkers, avbMorph,
...
`;
    ...
    if (!parsesAsModule(cfgPath) || !requireLoads(parserPath) || !requireLoads(previewHelperPath)) {
      pushDevLog(
