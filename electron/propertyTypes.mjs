...
export function createPropertyTypeReader(document) {
  const aliases = new Map();
  const duplicates = new Set();
  assert(
    document.statements.length <= PROPERTY_LIMITS.nodesMax,
    'Type declarations are bounded',
  );
...
function readPropertyType(type, aliases, duplicates) {
  const work = [{ node: type, aliases: [] }];
  const values = [];
  for (
    let count = 0;
  ...
  return resolved.length <= PROPERTY_LIMITS.textCharsMax
    ? resolved
    : type.getText();
}
function expandPropertyType(item, aliases, duplicates) {
