...
export function readPropertySyntax(source) {
  assert(
    source.length <= PROPERTY_LIMITS.sourceCharsMax,
    'Component source is bounded',
  );
  const match =
    /^(?:\uFEFF)?---[^\S\r\n]*\r?\n([\s\S]*?)^---[^\S\r\n]*(?:\r?\n|$)/m.exec(
      source,
...
export function replaceFrontmatter(document, frontmatter) {
  assert(
    document.end <= document.source.length,
    'Frontmatter ends inside source',
  );
  assert(
    frontmatter.length <= PROPERTY_LIMITS.sourceCharsMax,
    'New frontmatter is bounded',
  );
  if (document.start === 0) {
    return `---\n${frontmatter}\n---\n${document.source}`;
  }
  return (
    document.source.slice(0, document.start) +
    frontmatter +
    document.source.slice(document.end)
  );
}
...
export function syntaxError(source) {
...
export function validatePropertyCode(type, expression) {
  if (!type.trim()) {
  ...
  if (
    typeFile.statements.length !== 1 ||
    !declaration ||
    !ts.isTypeAliasDeclaration(declaration)
  ) {
    return err({
      code: 'type',
      message: 'Enter one TypeScript type, without declarations.',
    });
  }
...
export function syntaxNodes(root) {
  const pending = [root];
  const output = [];
...
export function isAstroProps(node) {
  if (!node) {
    return false;
  }
...
function isAstroPropsUnwrap(node) {
  let current = node;
...
export function propertyKey(node) {
  if (!node) {
    return undefined;
  }
...
export function defaultExpression(source) {
