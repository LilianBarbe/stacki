...
esbuild.buildSync({
  entryPoints: [path.join(__dirname, '..', 'src', 'projectPath.js')],
...
esbuild.buildSync({
  entryPoints: [path.join(__dirname, '..', 'src', 'ui', 'AssetThumb.jsx')],
...
esbuild.buildSync({
  entryPoints: [path.join(__dirname, '..', 'src', 'shortcutLabel.js')],
...
test('a missing native terminal binding does not prevent app startup', async () => {
  ...
  const terminalPath = require.resolve('../electron/terminal.js');
...
test('editing an Astro page preserves Windows line endings', () => {
  const { parsePage, serializePage } = require('../electron/astroParser');
  const source = '---\r\nconst title = "Hello";\r\n---\r\n<h1>{title}</h1>\r\n';
  const parsed = parsePage(source);
  assert.equal(parsed.editable, true);
  assert.equal(parsed.model.eol, '\r\n');
  const saved = serializePage(parsed.model);
  assert.ok(saved.includes('\r\n'));
  assert.doesNotMatch(saved, /(?<!\r)\n/);
  assert.equal(parsePage(saved).editable, true);
  const unix = serializePage(parsePage(source.replace(/\r\n/g, '\n')).model);
  assert.doesNotMatch(unix, /\r/);
});
